...
import {
  ...
  Upload, Download, Gift, ChevronLeft, ChevronRight, Lock, Save, BarChart2
} from 'lucide-react';
...
const CRM = () => {
  ...
  // Database states loaded from Supabase / localStorage
  ...
  const [childName, setChildName] = useState(() => localStorage.getItem('bumsy_parental_child_name') || 'Mateo');
  const [parentalLimit, setParentalLimit] = useState(() => localStorage.getItem('bumsy_parental_time_limit') || '30');
  const [parentalActive, setParentalActive] = useState(() => localStorage.getItem('bumsy_parental_active') === 'true');
  const [saveSuccess, setSaveSuccess] = useState(false);

  const handleSaveParentalControl = (e) => {
    e.preventDefault();
    localStorage.setItem('bumsy_parental_child_name', childName);
    localStorage.setItem('bumsy_parental_time_limit', parentalLimit);
    localStorage.setItem('bumsy_parental_active', parentalActive.toString());
    setSaveSuccess(true);
    triggerNotification('Configuración de control parental guardada con éxito.');
  ...
  useEffect(() => {
    ...
    const loadClients = async () => {
      try {
        const { data, error } = await supabase.from('clients').select('*').order('registered_at', { ascending: false });
    ...
    // 3. Fetch Inquiries from Supabase
  ...
  const handleConfirmPurchase = (e) => {
    ...
    if (isGiftProduct) {
      // 1. FREE GIFTS: Instant checkout directly registered in Supabase
      ...
      const newSale = {
        buyer_email: currentUser.email,
        buyer_name: currentUser.name,
        product_name: activeCheckoutProduct.name,
        price: activeCheckoutProduct.price,
        pdf_file: activeCheckoutProduct.pdfFile || activeCheckoutProduct.pdf_file || '',
        download_url: activeCheckoutProduct.downloadUrl || activeCheckoutProduct.download_url || '',
        address: addressVal,
        birthday: birthdayVal
      };

      const registerFreeSaleInSupabase = async () => {
        try {
  ...
  const handleSaveNotes = (clientId, notes) => {
    const saveNotesInSupabase = async () => {
      try {
        const { error } = await supabase
          .from('clients')
          .update({ notes })
  ...
  const handleImageUpload = (e) => {
    ...
    if (file) {
      if (file.size > 2 * 1024 * 1024) {
        triggerNotification('La foto excede los 2MB. Intenta con una imagen más optimizada.', 'error');
        return;
      }
      const reader = new FileReader();
      reader.onloadend = () => {
        setProdImageUrl(reader.result);
        triggerNotification('¡Foto cargada y procesada localmente con éxito!');
      };
  ...
  const handleProductSubmit = (e) => {
    ...
    const priceParsed = parseFloat(prodPrice) || 0;
    const isFreeVal = priceParsed === 0 || prodCategory === 'Regalos';
    ...
    if (editingProduct) {
      ...
      // Create mode
      const createProductInSupabase = async () => {
        try {
  ...
  const handleDeleteProduct = (prodId) => {
    const deleteProductInSupabase = async () => {
      try {
        const { error } = await supabase
          .from('products')
          .delete()
  ...
  const handleInquiryStatusChange = (inquiryId, newStatus) => {
    const updateInquiryStatusInSupabase = async () => {
      try {
        const { error } = await supabase
          .from('inquiries')
  ...
  const handleDeleteInquiry = (inquiryId) => {
    const deleteInquiryInSupabase = async () => {
      try {
        const { error } = await supabase
          .from('inquiries')
          .delete()
  ...
  const allProductsList = customProducts;
  ...
  useEffect(() => {
    ...
    if (paymentStatus === 'success') {
      ...
      if (!activeUser && emailParam) {
        const foundUser = clients.find(c => c.email.toLowerCase() === emailParam.toLowerCase());
        if (foundUser) {
      ...
      if (activeUser) {
        ...
        if (targetProd) {
          ...
          const registerPaidSaleInSupabase = async () => {
            try {
              ...
              // 2. Register sale in Supabase
              const newSale = {
                buyer_email: activeUser.email,
                buyer_name: activeUser.name,
                product_name: targetProd.name,
                price: targetProd.price,
                pdf_file: targetProd.pdfFile || targetProd.pdf_file || '',
                download_url: targetProd.downloadUrl || targetProd.download_url || '',
              ...
              // 3. Update active user details if pending purchase details exist
              if (pending) {
                await supabase
                  .from('clients')
                  .update({
  ...
  }, [sales, customProducts, clients, currentUser]);
          ...
          <div className="flex-1 flex flex-col gap-6">
            ...
            <div className="bg-slate-900/40 border border-slate-800 rounded-3xl p-6 flex flex-col md:flex-row items-center justify-between gap-4 backdrop-blur-md">
              ...
              <div className="flex items-center flex-wrap gap-2.5">
                ...
                <button
                  onClick={() => setBuyerTab('parental_control')}
                  className={`px-5 py-2.5 rounded-full text-xs font-black uppercase tracking-wider transition-all ${buyerTab === 'parental_control' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500/20 border border-indigo-500/30'}`}
                >
                  🛡️ Control Parental
                </button>
                <button
            ...
            {buyerTab === 'parental_control' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left w-full">
                {/* Left Column: Real Interactive Stats & Info */}
                <div className="bg-slate-900/30 border border-slate-800 rounded-3xl p-6 md:p-8 backdrop-blur-md flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <span className="text-[10px] bg-slate-950 border border-slate-800 text-slate-400 font-bold px-3 py-1 rounded-full uppercase tracking-wider">
                        Esta semana
                      </span>
                    </div>
                  ...
                  <div className="mt-8 pt-4 border-t border-slate-800 flex items-center gap-3 text-xs text-slate-400">
                    <UserCheck size={16} className="text-pink-500" />
                    <span>Cuenta de Padres: <strong>{currentUser.name}</strong></span>
                  </div>
                </div>
                ...
                <div className="bg-slate-900/30 border border-slate-800 rounded-3xl p-6 md:p-8 backdrop-blur-md flex flex-col justify-between">
                  <form onSubmit={handleSaveParentalControl} className="flex flex-col gap-6">
                    <div className="flex items-center justify-between">
                      <h3 className="text-lg font-black uppercase tracking-wider text-pink-400 flex items-center gap-2 font-black" style={{ fontFamily: "'Poppins', sans-serif" }}>
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="text-xs font-black uppercase tracking-wider text-slate-400 text-left">Nombre de tu Hijo(a):</label>
                      <input
                        type="text"
                        value={childName}
                        onChange={(e) => setChildName(e.target.value)}
                        className="bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-pink-500 font-semibold text-slate-250"
                    ...
                    <div className="bg-slate-950/40 border border-slate-800 p-4 rounded-xl text-xs text-slate-400 flex items-start gap-2.5 leading-relaxed" style={{ fontFamily: "'Poppins', sans-serif" }}>
                  ...
                  {saveSuccess && (
                    <motion.div
                      ...
                      className="bg-emerald-950/60 border border-emerald-500/30 text-emerald-400 px-4 py-3 rounded-xl text-xs font-bold mt-4 flex items-center gap-2"
                    >
                      <span>✨</span> ¡Configuración guardada en tu navegador!
                    </motion.div>
                  )}
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                  ...
                  <div className="xl:col-span-5 bg-slate-900/30 border border-slate-800 rounded-3xl p-6 h-fit backdrop-blur-md">
                    ...
                    <form onSubmit={handleProductSubmit} className="flex flex-col gap-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="text-xs font-bold text-slate-400 uppercase block mb-1.5 text-left">Foto del Producto (Subir o URL)</label>
                          <div className="flex flex-col gap-2">
                            {/* Local File Uploader */}
                            <label className="flex flex-col items-center justify-center border border-dashed border-slate-800 hover:border-indigo-500 rounded-xl p-3 cursor-pointer transition-colors bg-slate-950/40 group">
                            ...
                            <input
                              ...
                              className="w-full bg-slate-950 border border-slate-855 focus:border-indigo-500 rounded-xl px-3 py-2 text-xs focus:outline-none transition-colors text-slate-350 font-semibold"
                            />
                          </div>
                        </div>

                        <div>
                          <label className="text-xs font-bold text-slate-400 uppercase block mb-1.5 text-left">Vista Previa & Pre-diseños</label>
                          <div className="flex gap-2">
                            {/* Preview Window */}
                            <div className="w-1/2 aspect-video rounded-xl overflow-hidden bg-slate-950 border border-slate-850 flex items-center justify-center relative">
                              {prodImageUrl ? (
                                <img src={prodImageUrl} alt="Vista previa" className="w-full h-full object-cover" />
                              ) : (
                                <span className="text-[8px] uppercase tracking-wider font-bold text-slate-600">Sin foto</span>
                              )}
                              <div className="absolute top-1 left-1 bg-black/75 px-1.5 py-0.5 rounded text-[6px] uppercase tracking-wider font-bold text-slate-400">
                                Vista Previa
                              </div>
                            </div>
                            ...
                            <div className="w-1/2 grid grid-cols-3 gap-1">
                              {IMAGE_PRESETS.map((p, idx) => (
                                <button
                                  ...
                                  className={`aspect-video rounded-lg overflow-hidden border relative ${prodImageUrl === p.url ? 'border-indigo-500 shadow-md scale-102' : 'border-slate-850 opacity-60 hover:opacity-100 transition-opacity'}`}
                                  title={p.name}
                                >
                                  <img src={p.url} alt={p.name} className="w-full h-full object-cover" />
                                  <div className="absolute inset-0 bg-black/60 flex items-center justify-center p-0.5 text-[6px] uppercase tracking-wider font-semibold text-white text-center">
                                    {idx + 1}
                                  </div>
                                </button>
                              ))}
